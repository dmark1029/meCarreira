    ...
    if (mainBanner.length > 0) {
      setItems(
        mainBanner.map((item: any, index: number) => (
          <div
            onClick={() => {
              if (item.banner_link?.includes('?wallet=true&invitation=true')) {
                dispatch(showWalletForm({ invitationMode: true }))
              }
            }}
            className="seasons-rewards-section-banner-carousel"
            key={index}
          >
            <Link
              ...
              to={
                item.banner_link?.includes('?wallet=true&invitation=true')
                  ? '#'
                  : item.banner_link
              }
