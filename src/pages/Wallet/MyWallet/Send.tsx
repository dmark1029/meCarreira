...
const Send: React.FC<Props> = ({
  ...
  const submitForm = (values: any) => {
    ...
    try {
      const isAddress = ethers.utils.isAddress(values.address)
      if (isAddress) {
        isValidAddress = true
      }
    } catch (err: any) {
        ...
        <Formik
          ...
          validationSchema={Yup.object().shape({
            matic: Yup.string().required(
              assetType === 'playerCoin'
                ? t('player coin amount required')
                : `${assetType.toUpperCase()} ${t('amount required')}`,
            ),
            address: Yup.string()
              ...
              >
                    ...
                    <div className="nft-image-section">
                      <div className="image-border pc-avatar-container">
                        <ImageComponent
                          loading="lazy"
                          src={
                            playerData?.ticker === 'MATIC'
                              ? maticIcon
                              : playerData?.ticker === 'WETH'
                              ? ethereum
                              : playerData?.ticker === 'USDT'
                              ? tether
                              : playerData?.ticker === 'USDC'
                              ? usdc
                              : playerData?.ticker === 'WBTC'
                              ? bitcoin
                ...
                >
                  {playerData ? (
                    ...
                    <>
                      {!isInvalidEthAddress && isFundsInsufficient && (
                        <div className="input-feedback text-center fullwidth mt-40">
                          {t('insufficient Balance to make transaction')}
                        </div>
                      )}
                ...
                <div
                  className={classNames(
                    'bottom-button-box',
                    isMobile() ? 'wallet-submit-wrapper-mobile' : '',
                  )}
                >
                  {!loginInfo ? (
                    <div
