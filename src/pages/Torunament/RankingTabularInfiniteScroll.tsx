...
import { getTournamentPlayerRankingList } from '@root/apis/tournament/tournamentSlice'
...
const RankingTabularInfiniteScroll: React.FC = () => {
  ...
  const {
    ...
    tournamentListPlayerRankingLoading,
    updatingTournamentPlayerRankingList,
    tournamentPlayerRankingListNextUrl,
  } = useSelector((state: RootState) => state.tournament)

  // console.log(tournamentPlayerRankingList)
  ...
  const handleSearch = (value: string | undefined) => {
    ...
    dispatch(getTournamentPlayerRankingList(request))
  }
  ...
  const handleJumpToPage = (head: string) => {
    if (head !== 'back') {
      ...
      if (tournamentPlayerRankingListNextUrl) {
        setIsDeadEnd(false)
  ...
  useEffect(() => {
    ...
    if (tourStep === 'scout') {
      ...
      dispatch(getTournamentPlayerRankingList(leaderboardFilters))
          ...
          <div className="scouts-leaderboard-container">
            ...
            >
              ...
              >
                {tournamentListPlayerRankingLoading ? (
                ...
                ) : tournamentPlayerRankingList.length === 0 ? (
                  <div className="no-data-msg">{t('no data found')}</div>
                ) : (
                  <InfiniteScroll
                    style={{ overflow: 'hidden' }}
                    dataLength={tournamentPlayerRankingList.length}
                  ...
                  >
                    <div className="">
                      ...
                      >
                        {tournamentPlayerRankingList.map(
                        ...
                        {isDeadEnd ||
                        tournamentPlayerRankingList.length <
                          10 ? null : updatingTournamentPlayerRankingList ? (
...
export default RankingTabularInfiniteScroll
