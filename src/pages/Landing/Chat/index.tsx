...
function index({ isOpenChatModal, handleOpenChatModal }: Props) {
  ...
  useEffect(() => {
    if (curTab === 'general') {
      ...
      setMessageType('2')
      dispatch(getAllChats(2))
    }
    dispatch(getCredit())
  }, [curTab, isReply, isReplyToReply])
  ...
  const handleSubmitMessage = async () => {
    ...
    setTimeout(() => {
      dispatch(getAllChats(messageType));
      dispatch(getCredit())
      setIsReply(false)
      setIsReplyToReply(false)
      setFile(null)
  ...
  return (
    <div className="chat-container">
      {((isMobile() && isOpenChatModal) || !isMobile()) && (
        <div
        ...
        >
          ...
          {isMobile() ? (
            ...
            <>{isOpenChatModal ? <ExpandMoreIcon /> : <ExpandLessIcon />}</>
          )}
        </div>
      )}
      {isOpenChatModal && (
        ...
        <div className={`chat-content-contianer ${selectedImage ? " overflow-hidden" : ''}`}>
          <div className="chat-btn-container">
            <div
              ...
              className={curTab === 'general' ? 'active-btn' : 'inactive-btn'}
            >
              {t('general')}
            </div>
            <div
              onClick={() => handleChangeTab('feedback')}
              className={curTab === 'feedback' ? 'active-btn' : 'inactive-btn'}
            >
              {t('feedback')}
            </div>
          </div>
