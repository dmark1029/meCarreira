export const renderFireCanvas = () => {
  'use strict'
  try {
    // const canvas = document.getElementsByTagName('canvas')[0]
    const canvasList = document.getElementsByTagName('canvas')
    console.log({ canvasList })
    for (let k = 0; k < canvasList.length; k++) {
      console.log({ canvasList: canvasList[k] })
      canvasList[k].width = canvasList[k].parentElement.clientWidth
      canvasList[k].height = canvasList[k].parentElement.clientHeight
      var clickIndex = 0
      ...
      const waveContainer = document.getElementsByClassName('canvas_overlap')
      if (waveContainer.length > 0) {
        for (let l = 0; l < waveContainer.length; l++) {
          if (waveContainer[l]) {
            waveContainer[l].addEventListener('mousemove', color_hover)
          }
        }
      }
      ...
      const { gl, ext } = getWebGLContext(canvasList[k])
      ...
      function update() {
        // console.log('update', canvasList[k])
        if (!canvasList[k]) {
          return
        }
        resizeCanvas()
      ...
      function render(target) {
        ...
        if (target == null && config.TRANSPARENT) {
          ...
          gl.uniform1f(
            ...
            canvasList[k].width / canvasList[k].height,
          )
          blit(null)
        }
      ...
      function splat(x, y, dx, dy, color) {
        ...
        gl.uniform2f(
          ...
          x / canvasList[k].width,
          1.0 - y / canvasList[k].height,
        )
        gl.uniform3f(splatProgram.uniforms.color, dx, -dy, 1.0)
        if (k < 1) {
          gl.uniform1f(
            splatProgram.uniforms.radius,
            config.SPLAT_RADIUS_H / 100.0,
          )
        } else {
          gl.uniform1f(
            splatProgram.uniforms.radius,
      ...
      function multipleSplats(amount) {
        for (let i = 0; i < amount; i++) {
          ...
          color.g *= 10.0
          color.b *= 10.0
          const x = canvasList[k].width * Math.random()
          const y = canvasList[k].height * Math.random()
      ...
      canvasList[k].addEventListener(
        ...
        e => {
          ...
          for (let i = 0; i < touches.length; i++) {
            pointers[0].moved = pointers[0].down
            pointers[0].dx = (touches[i].pageX - pointers[0].x) * 8.0
            pointers[0].dy = (touches[i].pageY - pointers[0].y) * 8.0
            pointers[0].x = touches[i].pageX
            pointers[0].y = touches[i].pageY
          }
        },
        false,
      )

      canvasList[k].addEventListener('mousemove', e => {
        pointers[0].moved = pointers[0].down
      ...
      canvasList[k].addEventListener('mousedown', () => {
        pointers[0].down = true
        if (window?.location?.pathname === '/') {
          clickIndex = !clickIndex
          pointers[0].down = true
          if (Math.floor(Math.random() * 4) / 2 === 0) {
            pointers[0].color = { r: 0, g: 0.15, b: 0.0056628010708020815 }
          } else {
            pointers[0].color = { r: 0.15, g: 0.06362859243459015, b: 0 }
          }
        }
      })
      ...
      function generateColor() {
        if (clickIndex) {
          return { r: 0, g: 0.15, b: 0.0056628010708020815 }
        } else {
          return { r: 0.15, g: 0.06362859243459015, b: 0 }
        }
      }
